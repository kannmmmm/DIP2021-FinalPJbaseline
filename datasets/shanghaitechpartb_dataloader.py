...
def get_train_shanghaitechpartB_dataloader(labeled_file_list, use_flip, batch_size=1, mean=[0.5,0.5,0.5], std=[0.225,0.225,0.225]):
    main_transform_list = []
    if use_flip:
...
def get_test_shanghaitechpartB_dataloader(file_list):
    main_transform_list = []
    main_transform_list.append(PairedCrop())
    main_transform = torchvision.transforms.Compose(main_transform_list)
    image_transform = torchvision.transforms.Compose([
